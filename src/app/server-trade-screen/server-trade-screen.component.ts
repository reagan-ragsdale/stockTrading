...
export class ServerTradeScreenComponent implements OnInit {
  ...
  calculateBuyAndSellPoints(){
    ...
    for(let i = 0; i < this.selectedStockLast5.length; i++){
      if(buyOrSell == 'Buy'){
        if(((Math.abs(this.selectedStockLast5[i].avg - this.selectedStockLast40[i].avg)/ this.selectedStockLast40[i].avg) > .075) && ((Math.abs(this.selectedStockLast5[i].avg - this.selectedStockLast200[i].avg)/ this.selectedStockLast200[i].avg) < .1)){
