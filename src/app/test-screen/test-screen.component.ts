  ...
  async refreshData(data: DbCurrentDayStockData) {
    ...
    if (this.isUserOrBot == 'Bot' && this.isBotAuthorized == true) {
      ...
      if (this.selectedAlgo == 'highLow') {
        ...
        shouldPlaceOrder = AnalysisService.trendTrading(this.chartData.history.slice((this.chartData.history.length - this.trendAlgoStartingPoint) * -1), this.selectedStockHistoryData, this.stopLossPrice, this.tradeInitialAverage, this.tradeCurrentHigh, .45, .25)
      }
